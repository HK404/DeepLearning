...
print(torch.__version__)  # 1.8.1+cu102
...
# 指定输入、一组大小和一组宽高比，该函数将返回输入的所有锚框，数量为n+m−1
...
Y.shape  # torch.Size([1, 2042040, 4]) （1，锚框总个数，4）

# 将锚框变量y的形状变为（图像高，图像宽，以相同像素为中心的锚框个数，4）
boxes = Y.reshape((h, w, 5, 4))
# 这里打印一下坐标250，250的第一个锚框坐标
...
# 2、第二项为掩码（mask）变量，形状为(批量大小, 锚框个数的四倍)
# 掩码变量中的元素与每个锚框的4个偏移量一一对应 由于我们不关心对背景的检测，有关负类的偏移量不应影响目标函数
# 通过按元素乘法，掩码变量中的0可以在计算目标函数之前过滤掉负类的偏移量
print(labels[1])

# 3、返回的第一项是为每个锚框标注的四个偏移量，其中负类锚框的偏移量标注为0
print(labels[0])

# 使用非极大值抑制输出预测边界框
# 先构造4个锚框简单起见，我们假设预测偏移量全是0预测边界框即锚框。最后，我们构造每个类别的预测概率
...
# 这里为输入都增加了样本维。我们看到，返回的结果的形状为(批量大小, 锚框个数, 6)。其中每一行的6个元素代表同一个预测边界框的输出信息
# 第一个元素是索引从0开始计数的预测类别（0为狗，1为猫），其中-1表示背景或在非极大值抑制中被移除
# 第二个元素是预测边界框的置信度。剩余的4个元素分别是预测边界框左上角的xx和yy轴坐标以及右下角的xx和yy轴坐标（值域在0到1之间）
...
for i in output[0].detach().cpu().numpy():
    if i[0] == -1:
        continue
    # 小技巧[0]doge [1]catty
    label = ('doge=', 'catty=')[int(i[0])] + str(i[1])
